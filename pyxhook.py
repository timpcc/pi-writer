...
class HookManager(threading.Thread):
    ...
    def lookup_keysym(self, keysym):
        #print("Lookup: " + str(keysym))
        for name in dir(XK):
            # print(name + " attr: " + str(getattr(XK, name)))
            if name.startswith("XK_") and getattr(XK, name) == keysym:
                #print("NAME: "+ name)
                # lstrip will remove all occurances of the specified
                # characters which means keys 'X', 'K' and '_' will be removed
                # as well because entire string XK_X etc will be stripped.
                #val = name.lstrip("XK_")
                # instead do a 'remove first three characters' operation
                val = name[3:]
                #print("VAL: " + val)
                return val
        return "[%d]" % keysym
